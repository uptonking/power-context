...
# Export GLM environment variables from config
export REFRAG_RUNTIME GLM_API_KEY GLM_API_BASE GLM_MODEL

# Run ctx with collection
ENHANCED=$(timeout 30s python3 scripts/ctx.py "$USER_MESSAGE" --collection "$CTX_COLLECTION" 2>/dev/null || echo "$USER_MESSAGE")
