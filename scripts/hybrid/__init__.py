...
# Track whether we actually have the unified cache interface
_UNIFIED_CACHE_ACTIVE = False
if UNIFIED_CACHE_AVAILABLE:
    try:
        ...
        _UNIFIED_CACHE_ACTIVE = True
    except ImportError:
        _EMBED_CACHE = None
        _RESULTS_CACHE = None  # None signals fallback to _RESULTS_CACHE_OD
...
def run_hybrid_search(
    ...
    if model:
        ...
        # Use the re-exported get_embedding_model which handles factory/fallback consistently
    ...
    if _USE_CACHE:
        ...
        if cache_key is not None:
            if _UNIFIED_CACHE_ACTIVE and _RESULTS_CACHE is not None:
    ...
    if not merged:
        if _USE_CACHE and cache_key is not None:
            ...
            # Always store in fallback OD for deterministic test hits
            with _RESULTS_LOCK:
